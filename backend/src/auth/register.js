...
module.exports = (app) => {
    app.post('/api/profile/register', async (req, res) => {
        ...
        await Users.create({ user_login: login, user_password: passwordHash, user_email: email, user_avatar: avatar })

        const profile = await Users.findOne({
            where: { user_login: login },
            attributes: ['user_id', 'user_login', 'user_email', 'user_avatar', 'user_balance', 'user_avatar', 'user_email', 'user_receiveInfo', 'user_role' ],
        }).then((user) => user)
