...
const {
    isAcceptablePassword, isAcceptableLogin, normalizeEmail, isAcceptableEmail,
} = require('./credentials');
...
module.exports = (app) => {
    app.post('/api/profile/register', authLimiter, async (req, res) => {
        try {
            const {
                login, password, email: rawEmail, avatar,
            } = req.body;

            if ([login, password, rawEmail].some((value) => typeof value !== 'string' || !value)) {
                return res.status(401).json({ message: message.AUTH.EMPTY_DATA });
            }

            if (!isAcceptableLogin(login)) {
                return res.status(422).json({ message: message.AUTH.LOGIN_INVALID });
            }
            ...
            if (!isAcceptableEmail(email)) {
                return res.status(422).json({ message: message.AUTH.EMAIL_INVALID });
            }
